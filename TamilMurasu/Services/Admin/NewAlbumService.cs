...
namespace TamilMurasu.Services.Admin
{
    public class NewAlbumService : INewAlbumService
    {
        ...
        public DataTable GetAllNewAlbum(string strStatus)
        {
            ...
            {
                SvSql = "select I_id,S_Image,L_image,Foot_Note,News_head,Addeddate,I_cat from TMImages_N where I_cat='22' and deletenews='Y' and I_Cid=0 order by I_id desc";
            }
            else
            {
                SvSql = "select  I_id,S_Image,L_image,Foot_Note,News_head,Addeddate,I_cat from TMImages_N where I_cat='22' and deletenews='N' and I_Cid=0 order by I_id desc ";
            }
                ...
                using (SqlConnection objConn = new SqlConnection(_connectionString))
                {
                    ...
                    if (Cy.ID == null)
                    {
                        if (files != null && files.Count > 0)
                        {
                            ...
                            svSQL = "Insert into TMImages_N (I_cat,I_Cid,S_Image,L_image,Foot_Note,publish_up,publish_down,News_head,deletenews,most_view,tag,AddedDate) VALUES ('22','0','" + filename1 + "','0','" + Cy.Album + "','','','" + Cy.EnglishAlbum + "','Y','0','0','" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "')";
                    ...
                    {
                        svSQL = "Update TMImages_N set Foot_Note = '" + Cy.Album + "',News_head = '" + Cy.EnglishAlbum + "' WHERE TMImages_N.I_Id ='" + Cy.ID + "'";
                        SqlCommand objCmds = new SqlCommand(svSQL, objConn);
                        objCmds.ExecuteNonQuery();
                    }
                    objConn.Close();
        ...
        {
            string SvSql = string.Empty;
            SvSql = "select I_Id,Foot_Note,News_head from TMImages_N  Where TMImages_N.I_Id='" + id + "' ";
            DataTable dtt = new DataTable();
            SqlDataAdapter adapter = new SqlDataAdapter(SvSql, _connectionString);
            SqlCommandBuilder builder = new SqlCommandBuilder(adapter);
            adapter.Fill(dtt);
