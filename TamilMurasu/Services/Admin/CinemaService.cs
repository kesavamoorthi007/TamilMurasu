...
namespace TamilMurasu.Services.Admin
{
    public class CinemaService : ICinemaService
    {
        ...
        public DataTable GetAllCinema(string strStatus)
        {
            ...
            {
                SvSql = "select I_id,S_Image,L_image,Foot_Note,News_head,deletenews,Addeddate,I_cat from TMImages_N where I_cat='25' and  deletenews='Y' and I_Cid=0 order by I_id desc";
            }
            else
            {
                SvSql = "select I_id,S_Image,L_image,Foot_Note,News_head,deletenews,Addeddate,I_cat from TMImages_N where I_cat='25' and  deletenews='N' and I_Cid=0 order by I_id desc";
            }
                ...
                using (SqlConnection objConn = new SqlConnection(_connectionString))
                {
                    ...
                    if (Cy.ID == null)
                    {
                        if (files != null && files.Count > 0)
                        {
                            ...
                            svSQL = "Insert into TMImages_N (I_cat,I_Cid,S_Image,L_image,Foot_Note,publish_up,publish_down,News_head,deletenews,most_view,tag,Addeddate) VALUES ('25','0','" + filesave1 + "','0',N'" + Cy.Album + "','','','" + Cy.EnglishAlbum + "','Y','0','" + Cy.Tag + "','" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "')";
        ...
        {
            string SvSql = string.Empty;
            SvSql = "select I_Id,Foot_Note,S_Image,News_head,tag from TMImages_N  Where TMImages_N.I_Id='" + id + "' ";
            DataTable dtt = new DataTable();
            SqlDataAdapter adapter = new SqlDataAdapter(SvSql, _connectionString);
            SqlCommandBuilder builder = new SqlCommandBuilder(adapter);
            adapter.Fill(dtt);
