...
namespace TamilMurasu.Services.Admin
{
    public class LatestNewsService : ILatestNewsService
    {
        ...
        public DataTable GetAllLatestNews(string strStatus)
        {
            ...
            {
                SvSql = "select  I_Id,Foot_Note,deletenews,CONVERT(varchar, TMImages_N.publish_up, 106) AS AddedDateFormatted,CONVERT(varchar, TMImages_N.publish_down, 106) AS AddedDateFormatted1,News_head from TMImages_N WHERE I_cat='21'  and TMImages_N.deletenews='N' ORDER BY TMImages_N.I_Id DESC";
            }
        ...
        public string LatestNewsCRUD(LatestNews Cy)
        {
            ...
            try
            {
                ...
                using (SqlConnection objConn = new SqlConnection(_connectionString))
                {
                    ...
                    if (Cy.ID == null)
                    {
                        svSQL = "Insert into TMImages_N (I_cat,I_Cid,S_Image,L_image,Foot_Note,Addeddate,publish_up,publish_down,News_head,deletenews,most_view,tag) VALUES ('21','21','0','0',N'" + Cy.NewsDetail + "','" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "','" + Cy.PublishUp + "','" + Cy.PublishDown + "',N'" + Cy.NewsHead + "','Y','0','0')";
                    ...
                    {
                        svSQL = "Update TMImages_N set Foot_Note = N'" + Cy.NewsDetail + "',publish_up = '" + Cy.PublishUp + "',publish_down = '" + Cy.PublishDown + "',News_head = N'" + Cy.NewsHead + "' WHERE TMImages_N.I_Id ='" + Cy.ID + "'";
                        SqlCommand objCmds = new SqlCommand(svSQL, objConn);
                        objCmds.ExecuteNonQuery();
                    }
                    objConn.Close();
